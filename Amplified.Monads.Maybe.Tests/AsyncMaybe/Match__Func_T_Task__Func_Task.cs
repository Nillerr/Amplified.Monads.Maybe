...
namespace Amplified.Monads.Maybe
{
    ...
    public class AsyncMaybe__Match__Func_T_Task__Func_Task
    {
        ...
        public async Task NoneValue_WithLambdas()
        {
            ...
            await source.Match(_ => { some++; return TaskCache.CompletedTask; }, () => { none++; return TaskCache.CompletedTask; });
            Assert.Equal(0, some);
            Assert.Equal(1, none);
        }
        ...
        public async Task WithSomeLambda_AndNoneReference()
        {
            ...
            Task MatchNone() { none++; return TaskCache.CompletedTask; };
        ...
        public async Task WithSomeReference_AndNoneLambda()
        {
            ...
            var source = AsyncMaybe<int>.Some(1);
            await source.Match(someAsync: MatchSome, noneAsync: () => { none++; return TaskCache.CompletedTask; });
            Assert.Equal(1, some);
            Assert.Equal(0, none);
        }
